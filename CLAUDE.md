...
- **`orchestration.handoff` / `orchestration.handoffctl` / `orchestration.handoff_registry` / `orchestration.handoff_watcher`** — standard-library local-v1 durable handoff protocol plus private run and coordinator-session state. Generic `watch` remains a JSONL observer; detached coordinator mode dynamically discovers only owned runs, persists credential-free notification cursors, coalesces opaque doorbells without consuming worker events, and binds its lifetime to the exact orchestrator PID plus process-start identity. cmux coordinator doorbells pair an echo-confirmed typed prompt into the coordinator surface — the only channel that pushes a hosted agent to act; a zero-exit `cmux send` alone proves nothing — with a visible `cmux notify` alert for the human; the watcher records the accepted channel(s) in `last_doorbell_method` (e.g. `cmux_input+cmux_notify`), never confirmed user visibility. The cmux server rejects socket calls from launchd-orphaned clients under the default `socketControlMode: cmuxOnly`, so cmux coordinators default to a **surface-hosted** watcher (`coordinator start --mode auto|detached|surface`): a dedicated in-tree cmux terminal surface (named `watcher: <workspace>`, parked in a bottom `handoff-watchers` workspace) runs the watch loop in its foreground, native `cmux notify` doorbells work, and the tab is the watcher's visible log — closing it kills the watcher and `coordinator start` relaunches it. tmux coordinators keep the detached daemon and typed doorbells; an explicitly detached cmux watcher degrades to a recorded `macos_notification`. Coordinator `send` rings the worker's terminal doorbell by default for registered local runs (best-effort, reported as `doorbell_sent`/`doorbell_error`; `--no-doorbell` opts out), while `dispatch` remains the one-shot steer for callers holding no active lease.
- **`orchestration.handoff_launcher` / `scripts/handoff_agent.sh`** — safe local cmux/tmux and remote SSH+tmux launcher for Claude Code (`opus/high`), Codex (`gpt-5.6-terra/xhigh`), and Kimi Code (`kimi-code/k3`/`max`), with model/effort overrides; remote requests carry kickoff bytes as JSON over SSH stdin and keep the authoritative run on the worker host. Launch-only returns without a readiness wait and releases its lease, while managed monitoring is opt-in. Companion helper scripts (details: docs/architecture.md): `scripts/handoff_coordinator_ensure.sh` (idempotent watcher bootstrap), `scripts/handoff_doorbell.sh` (manual doorbell with submit discipline), `scripts/cmux_close_surface_safe.sh` (guarded single-surface close), `scripts/tmux_run_logged.sh` (logged batch session with live pane).
