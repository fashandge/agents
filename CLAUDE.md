...
- **`orchestration.handoff` / `orchestration.handoffctl` / `orchestration.handoff_registry` / `orchestration.handoff_watcher`** — standard-library local-v1 durable handoff protocol plus private run and coordinator-session state. Generic `watch` remains a JSONL observer; coordinator mode dynamically discovers only owned runs, persists credential-free notification cursors, and coalesces opaque doorbells without consuming worker events.
