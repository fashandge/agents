...
- **`orchestration.handoff` / `orchestration.handoffctl` / `orchestration.handoff_registry` / `orchestration.handoff_watcher`** — standard-library local-v1 durable handoff protocol plus private run and orchestrator-session state. Generic `watch` remains a JSONL observer; detached orchestrator mode dynamically discovers only owned runs, persists credential-free notification cursors, coalesces opaque doorbells without consuming worker events, and binds its lifetime to the exact orchestrator PID plus process-start identity. cmux orchestrator doorbells pair an echo-confirmed typed prompt into the orchestrator surface — the only channel that pushes a hosted agent to act; a zero-exit `cmux send` alone proves nothing — with a visible `cmux notify` alert for the human; the watcher records the accepted channel(s) in `last_doorbell_method` (e.g. `cmux_input+cmux_notify`), never confirmed user visibility. The typed channel is gated on the orchestrator's composer so a doorbell never lands mid-draft: an empty composer sends now, a draft on an unfocused surface is forced with a self-describing draft-preservation prefix (`cmux_input_forced`), and a draft on the focused surface defers (`deferred_input`, retried next poll) until it stops changing. The cmux server rejects socket calls from launchd-orphaned clients under the default `socketControlMode: cmuxOnly`, so cmux orchestrators default to a **surface-hosted** watcher (`orchestrator start --mode auto|detached|surface`): a dedicated in-tree cmux terminal surface (named `watcher: <workspace>`, parked in a bottom `handoff-watchers` workspace) runs the watch loop in its foreground, native `cmux notify` doorbells work, and the tab is the watcher's visible log — closing it kills the watcher and `orchestrator start` relaunches it. tmux orchestrators keep the detached daemon and typed doorbells; an explicitly detached cmux watcher degrades to a recorded `macos_notification`. Orchestrator `send` rings the worker's terminal doorbell by default for registered local runs (best-effort, reported as `doorbell_sent`/`doorbell_error`; `--no-doorbell` opts out), while `dispatch` remains the one-shot steer for callers holding no active lease.
