...
- End the turn after a blocking question. On completion publish an exact result, then end the turn and await review. After a successful result publication, `handoffctl` attempts a best-effort cmux notification for cmux-launched workers; the durable result remains authoritative if notification delivery fails.
- After consuming an accepted review, emit a `paused` checkpoint and end the turn, then idle awaiting further instructions — a doorbell will arrive with the next one. Resume from paused only after consuming a `steer`, `answer`, `supersede`, `input-changed`, or `base-changed` newer than the accepted review: checkpoint back to `working` and continue. There is no lifecycle message and no exit checkpoint: the run ends when the orchestrator marks it finished and reaps the session.
- Waiting is always passive: end the turn and stop working. Never poll, watch, tail, sleep against, or re-read the inbox in a loop to wait for a review, an answer, or the next instruction, and never record such a wait as a commitment. The doorbell wakes you, and it cannot interrupt a turn that is still running, so an active wait only delays the thing it is waiting for. Read the inbox at the checkpoints above and nowhere else.
